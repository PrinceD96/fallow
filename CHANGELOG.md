...
- **`fallow viz` renders your codebase as an interactive analysis map.** A new command runs one shared project analysis and writes a self-contained HTML file with a nested treemap and force-directed import graph. Six primary lenses cover Overview, Unused, Duplication, Architecture, retained Health metrics, and static Security candidates. An adaptive More menu exposes Dependencies, Frameworks, Styling, and Feature flags without crowding the header. Every analysis distinguishes complete, disabled, not applicable, and unavailable states, and labels counts by unit so missing data never looks like zero findings. The file panel leads with the active lens and keeps evidence, traces, trust-boundary context, and verification actions beside the finding they affect. Runtime Security remains explicitly unavailable when no runtime evidence is present, and static candidates are never presented as confirmed vulnerabilities. Search, keyboard shortcuts, shareable URL state, legacy lens links, dark and light themes, reduced motion, and honest unavailable Components, Packages, and diff scopes are built in. The HTML opens in your browser by default (`--no-open` to skip, `--out <path>` to choose the file); `--viz-format dot` and `--viz-format mermaid` emit the import graph as text. Read-only, and respects `--production`, `--config`, and `--no-cache` like the analysis commands.
